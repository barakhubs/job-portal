...
$user = $func->selectSingle($auth, 'candidates');
...
if (isset($_POST['submit'])) {
    ...
    if($old_pw != "" || $new_pw != "" || $conf_pw !=""){
        if (strlen($new_pw)<6) {
            header("Location:profile.php?error=Password to short! Try again");

            exit();
        }elseif($new_pw === $conf_pw){
            if (!password_verify($old_pw, $user['password'])) {
                header("Location:profile.php?error=Password does not match old password! Try again");
            ...
            header("Location:profile.php?error=Passwords do not match! Try again");
